...
class Gui:
    ...
    def __init__(self):
        ...
        def setRadioButton(button):
            ...
            """
            global current_item
            app.setLabel("counter", dict[app.getRadioButton('verb_1')]['count'])
        ...
        app.addButton('-', self.press)
        app.addLabel("counter", dict['ice cream']['count'])
    ...
    def press(self, button):
        ...
        def innerPress(button2):
            ...
            if button2 == "Correct":
                ...
                for key, value in enumerate(dict):
                    print(value + ':', str(dict[value]['count'] * dict[value]['price']))
                    total += dict[value]['count'] * dict[value]['price']
                print('Total:', total)
                app1.stop()
        ...
        if button == "Submit":
            ...
            for key, value in enumerate(dict):
                cost_string += value + ' x ' + str(dict[value]['count']) + '= $' + str(
                    dict[value]['count'] * dict[value]['price']) + '\n'
                total += dict[value]['count'] * dict[value]['price']
            print('\n\n')
            # stop gui
            self._app.stop()
            ...
            dict[current_item]['count'] += 1
            self._app.setLabel("counter", dict[current_item]['count'])
        elif button == "-":
            # self._app.setRadioButton("minus", )
            if dict[current_item]['count'] != 0:
                dict[current_item]['count'] -= 1
                self._app.setLabel("counter", dict[current_item]['count'])
